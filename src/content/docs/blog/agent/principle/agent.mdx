...
createAt: 2026-04-09
screen: images/agent-20260301.webp
sidebarTitle: 原理
homeFeed: true
---

> 原文地址：[https://tw93.fun/2026-03-21/agent.html](https://tw93.fun/2026-03-21/agent.html)

![你不知道的 Agent：原理、架构与工程实践](./images/agent-20260301.webp)
...
对应的控制流如下，`感知 -> 决策 -> 行动 -> 反馈`（`PDAF`）四个阶段不断循环，直到模型返回纯文本为止：

![控制流](./images/react_loop_control_flow.svg)
...
![Workflow 和 Agent 有什么区别](./images/workflow_vs_agent.svg)
...
![五种常见控制模式](./images/five_agent_patterns.svg)
...
![OpenAI 的 Agent 优先开发实践](./images/OAI_Harness_engineering_Giving_Codex_a_full_observability_stack_desktop-light__1_.svg)
...
![Harness 的关键结论是什么](./images/svgviewer-output.svg)
...
![上下文为什么要分层](./images/context_layers.svg)
...
![为什么 Skills 要按需加载](./images/WQUEG4.webp)
...
![betaZodTool](./images/aci_tool_design1.svg)
...
![四种记忆分别存在哪里](./images/agent_memory_types.svg)
...
![记忆整合如何触发并回退](./images/memory_consolidation_.svg)
...
![长任务如何跨 session 继续](./images/conding-agent.svg)
...
![多 Agent 如何组织](./images/agent-20260301-02.svg)
...
![为什么协作方式要写成协议](./images/multi_agent_protocol.svg)
...
![多 Agent 下幻觉会互相放大](./images/svgviewer-output-01.svg)
...
![为什么 Agent 评测结构更复杂](./images/L7tEpR.webp)
...
![为什么 Agent 评测结构更复杂2](./images/YpOEZB.webp)
...
![评测现状与常用指标1](./images/H5Cn8z.webp)
![评测现状与常用指标2](./images/Al2yJz.webp)

上图（上方）是评测方式，下图（上方）是常用指标，人工标注和 LLM judge 加起来占主导，传统 ML 指标只有 16.9%，还有近四分之一的团队还没开始做评测。
...
![先修评测，再改 Agent](./images/ERIBy8.webp)
...
![两层可观测性如何分工](./images/observability_two_layer59.svg)
...
![事件流为什么更适合做底座](./images/svgviewer-output-2.svg)
...
![长任务时的身份重申](./images/svgviewer-output-3.svg)
